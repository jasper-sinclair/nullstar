  ...
  if (selfplay_mode && time.use_depth_limit){
    ...
    while (td.root_depth < max_depth){
      ...
      for (;;){
        ...
        if (time.stop.load(std::memory_order_relaxed))
          break;
...
template <search_type st, bool skip_hash_move> int search::alpha_beta(
  ...
  if (time.stop.load(std::memory_order_relaxed)) return stop_score;
  ...
  const int hash_score =
    hash_table::score_from_hash(he.score,ss->ply);
  if (!pv_node && !skip_hash_move && hash_hit && depth <= he.depth){
    if (he.nt == pvnode ||
      (he.nt == cutnode && hash_score >= beta) ||
      (he.nt == allnode && hash_score <= alpha)){
  ...
  if (is_in_check) eval = ss->static_eval = -infinite_score;
    ...
    ss->static_eval = he.eval;
    if (depth <= he.depth &&
      (he.nt == pvnode ||
        (he.nt == cutnode && hash_score > ss->static_eval) ||
        (he.nt == allnode && hash_score < ss->static_eval))){
      eval = hash_score;
  ...
  const u16 hash_move = hash_hit ? he.move : u16();
...
template <search_type st> int search::quiescence(
  ...
  hash_data he;
  const bool hash_hit = hash.probe(key,he);
  const int hash_score = hash_table::score_from_hash(he.score,ss->ply);
  if (!pv_node && hash_hit &&
    (he.nt == pvnode || (he.nt == cutnode && hash_score >= beta) ||
      (he.nt == allnode && hash_score <= alpha)))
    return hash_score;
  const bool is_in_check = pos.is_in_check();
  int best_score;
  if (is_in_check) best_score = ss->static_eval = -infinite_score;
  else best_score = ss->static_eval = hash_hit ? he.eval : nnue::evaluate(pos);
  if (hash_hit && (he.nt == pvnode ||
    (he.nt == cutnode && hash_score > best_score) ||
    (he.nt == allnode && hash_score < best_score)))
  ...
  u16 best_move = hash_hit ? he.move : u16();
...
void search::stop(){
  time.stop.store(true,std::memory_order_relaxed);
}

void search::set_hash_size(
  const size_t mb){
  hash.set_size(mb);
}

u64 search::node_count() const{
  u64 sum = 0;
...
void search::clear(){
  hash.clear();
  thread_info.clear();
  for (thread_id i = 0; i < num_threads; ++i)
    thread_info.push_back(std::make_unique<thread_data>(i));
}

void search::set_num_threads(
  const thread_id threadnum){
  this->num_threads = std::clamp(threadnum, SCTI(1),
    std::min(std::thread::hardware_concurrency(),
      SCTI(max_threads)));
  thread_info.clear();
  for (thread_id i = 0; i < this->num_threads; ++i)
    thread_info.push_back(std::make_unique<thread_data>(i));
}
