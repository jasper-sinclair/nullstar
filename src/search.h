...
struct thread_data{
  explicit thread_data(
    const thread_id id) : root_depth(0), stack {}, id(id), node_count(0){}
...
struct search{
  ...
  std::vector<std::unique_ptr<thread_data>> thread_info;
...
extern template u16 search::best_move<true>(
  board& pos,
  thread_id id);
extern template u16 search::best_move<false>(
  board& pos,
  thread_id id);
