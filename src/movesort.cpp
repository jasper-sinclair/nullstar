...
void move_sort::compute_scores(){
  ...
  const bitboard threatened_pieces =
    ...
    ((threatened_by_pawn &
      (position.get_pieces(knight) | position.get_pieces(bishop))) |
...
void continuation_hist::increase(
  const board& pos,
  const search_stack* stack,
  const u16 move,
  const i32 depth){
  if ((stack - 1)->move){
    auto& e1 = data()[(stack - 1)->moved][move::to((stack - 1)->move)]
      [pos.piece_on(move::from(move))][move::to(move)];
    e1 += heinc1 * p(depth) * (max - e1) / max;
    if ((stack - 2)->move){
      auto& e2 = data()[(stack - 2)->moved][move::to((stack - 2)->move)]
        [pos.piece_on(move::from(move))][move::to(move)];
      e2 += heinc2 * p(depth) * (max - e2) / max;
      if ((stack - 4)->move){
        auto& e4 = data()[(stack - 4)->moved][move::to((stack - 4)->move)]
          [pos.piece_on(move::from(move))][move::to(move)];
        e4 += heinc4 * p(depth) * (max - e4) / max;
      }
    }
  }
}

void continuation_hist::decrease(
  const board& pos,
  const search_stack* stack,
  const u16 move,
  const i32 depth){
  if ((stack - 1)->move){
    auto& e1 = data()[(stack - 1)->moved][move::to((stack - 1)->move)]
      [pos.piece_on(move::from(move))][move::to(move)];
    e1 -= hedec1 * p(depth) * (max + e1) / max;
    if ((stack - 2)->move){
      auto& e2 = data()[(stack - 2)->moved][move::to((stack - 2)->move)]
        [pos.piece_on(move::from(move))][move::to(move)];
      e2 -= hedec2 * p(depth) * (max + e2) / max;
      if ((stack - 4)->move){
        auto& e4 = data()[(stack - 4)->moved][move::to((stack - 4)->move)]
          [pos.piece_on(move::from(move))][move::to(move)];
        e4 -= hedec4 * p(depth) * (max + e4) / max;
      }
    }
  }
}
