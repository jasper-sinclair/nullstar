...
struct bitboard{
  ...
  [[nodiscard]] constexpr bool is_set(
    const u8 sq) const{
...
namespace move{
  ...
  constexpr move_type mt(
    const u16 m){
    return SCMT(m & 0x3 << 14);
  }

  constexpr i32 get_piece_type(
    const u16 m){
    return (m >> 12 & 0x3) + 2;
  }
...
inline board_state* board::bs(
  const int idx){
  return &board_status[board_status.size() - idx - 1];
}

inline bool board::can_castle(
  const int cr) const{
...
inline bitboard board::get_color(
  const bool c) const{
  return color_bb[c];
}

inline bitboard board::get_pieces(
  const i32 pt) const{
  return piece_bb[pt];
}
...
inline bool board::is_promotion(
  const u16 m){
  return move::mt(m) == move::promotion;
}

inline bitboard board::non_pawn_material(
  const bool c) const{
...
inline int popcnt(
  const bitboard b){
  return __builtin_popcountll(b.data);
}
inline u8 lsb(
  const bitboard b){
  return u8(__builtin_ctzll(b.data));
}
#endif
inline u8 pop_lsb(
  bitboard& b){
